...
Please provide a valid "-out" root directory.
...
# reformatting the publication entries & exporting them to "-out"
# using the
foreach my $pgPMID (@$pgPMIDs) {
  ...
  if ($pub->{CANCERTYPE} =~ /^seer\s(21049)\:\s*?(\w.*?)$/) {
    $pubDump->{cancertype}      =   {
      term_id           =>  'seer:'.$1,
      term_label        =>  $2,
    };
